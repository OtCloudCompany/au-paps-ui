...
import { ModalContentComponent } from '../untyped-item/modal-content/modal-content.component';
import {
  Cite,
  plugins,
...
export class PublicationComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.object.hasMetadata(['dspace.entity.type'])) {
      ...
      console.log(this.object.allMetadataValues(['dc.type']));
    } else {
    ...
    this.shareLinks = {
      ...
      twitter: `https://twitter.com/intent/tweet?text=${this.itemTitle}&url=${this.handleIdentifier}`,
      linkedin: `https://www.linkedin.com/shareArticle?mini=true&url=${this.handleIdentifier}&title=${this.itemTitle}&summary=${this.itemAbstract}`,
      email: `mailto:?subject=${encodeURIComponent(`Check out this Journal Article: ${this.itemTitle}`)}&body=${encodeURIComponent(
        `Interesting Journal Article: ${this.itemTitle}\n\nRead it here: ${this.handleIdentifier}`,
      )}`,
    };
  ...
  generateCitations() {
    ...
    const cite = new Cite({
      ...
      journal: this.object.firstMetadataValue(['dc.relation.journal']),
      volume: this.object.firstMetadataValue(['dc.relation.volume']),
      issue: this.object.firstMetadataValue(['dc.relation.issue']),
      issued: {
  ...
  convertToCSV(cite) {
    ...
    const headers = ['Type', 'Title', 'Author', 'Date', 'Diario', 'Editora', 'URL'];
