...
@Component({
  selector: 'ds-knowledge-wheel',
  standalone: true,
  imports: [
    HighchartsChartModule,
    NgIf
  ],
  templateUrl: './knowledge-wheel.component.html',
  styleUrl: './knowledge-wheel.component.scss',
})
export class KnowledgeWheelComponent extends TopLevelCommunityListComponent implements OnInit {
    ...
    this.communitiesRD$.subscribe(( (dataFetched) =>{
      ...
      if ( dataFetched.hasSucceeded ){
        ...
        this.communitiesList.forEach((community: Community, index)=>{
          ...
          const communityUrl = community.metadata['dc.identifier.uri'][0].value;
